...
export const useFetch = () => {
  const handleFetch = async (
    ...
    const requestUrl = request?.params ? `${process.env.NEXT_PUBLIC_BASE_URL ? (process.env.NEXT_PUBLIC_BASE_URL + '/'): ''}${url}${request.params}` : `${process.env.NEXT_PUBLIC_BASE_URL ? (process.env.NEXT_PUBLIC_BASE_URL + '/'): ''}${url}`;
